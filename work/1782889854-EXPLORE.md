# EXPLORE — LABEL the A21 DISTILL (four nodes) + Review: no parity break; PAUSE

**Mode:** EXPLORE
**Timestamp:** 1782889854
**Branch:** claude/epic-wozniak-53e47f
**This step (two parts, per protocol):**
- **Part 0 — LABELLING (first act):** the latest `work/` file is an *unlabelled* DISTILL
  (`work/1782889324-DISTILL.md`) that split the two A21 candidate lemmas `L-DDS` and `L-TRACE-REFORM`
  into **four** load-bearing nodes. I independently re-verify the load-bearing audits (I do **not**
  rubber-stamp DISTILL's pre-fills), then assign each node its status and move it into the Claim Graph /
  Graveyard, folding the A21 verdict into the tracking sheet.
- **Part 1 — NAMED ACTION: Review.** Re-survey the sheet for the A21 disposition and the notable
  structural yield (the DDS-CORE / C7 unification), record the escalation handles, and state the
  recommendation.

> **Anti-deception, weighted first (binding).** `[DDS-CORE]` is marked NEEDS-REVIEW **only** as a rigorous
> wall-FACT about **finite ℤ/2 geometric local systems** (with a reproducible audit) — it is **NOT a
> foundation toward TPC, NOT a bound, NOT progress.** `[TRACE-SUFF]` is ESTABLISHED but is **only a
> sufficient condition** (it does not produce the operator, so it is not a bound). `[DDS-ENV]` and
> `[TRACE-EQUIV]` are **OPEN** — no construction / no impossibility proof, and no structural `A_x` (no
> audit for a bound); marking either NEEDS-REVIEW on plausibility is exactly the forbidden move. I do not
> relabel an unbroken obstruction as progress. The honest sentence stands: **no progress on C5 that
> survives audit; the wall now has a rigorous geometric-non-realizability core (tied to C7) and a precise
> spectral reformulation, both short of a bound.** No citations invented (all re-verified below).

---

## Part 0 — LABELLING (independent re-verification, then assignment)

### 0.0 What DISTILL handed back

`work/1782889324-DISTILL.md` reduced the two two-name candidate lemmas to their smallest load-bearing
splits (four nodes) and left the NEEDS-REVIEW-vs-FALSIFIED verdict to me, with pre-fills:

- `L-DDS` → **[DDS-CORE]** (rigorous: λ is not a finite-order idele-class Dirichlet/Hecke character ⇒ no
  finite geometric ℤ/2 local system on any open of `Spec ℤ` has monodromy λ) + **[DDS-ENV]** (heuristic
  envelope: no structured theory carries BOTH ε and +2 with a trace). DISTILL: DDS-CORE **survived**,
  corroborated a 2nd way (arithmetic-site zeta = complete Riemann ζ ⇒ Trap A); DDS-ENV OPEN.
- `L-TRACE-REFORM` → **[TRACE-SUFF]** (⟸ = Lidskii) + **[TRACE-EQUIV]** (⟹ under the structural clause).
  DISTILL: TRACE-SUFF = exactly Lidskii; TRACE-EQUIV ⟹ downgraded/unproven.

Per the anti-deception rule I re-verify the load-bearing audits myself before assigning.

### 0.1 Re-verification of the computational audits (run fresh this step)
...
# RE-VERIFY 1: DDS-CORE facts (lambda's series; lambda(p)=-1; no period)
ok_local = True
...
print("1a  (1+p^-s)^-1 == (1-p^-s)/(1-p^-2s)  [=> Euler product = zeta(2s)/zeta(s)]:", ok_local)
M = 400000
...
for s in [2.0, 3.0]:
    approx = float(np.sum(lam[1:]*nn**(-s))); exact = float(zeta(2*s)/zeta(s))
    print("1b  s=%.0f  sum lam(n)/n^s=%.10f  zeta(2s)/zeta(s)=%.10f  |diff|=%.2e"
          % (s, approx, exact, abs(approx-exact)))
lp = [(p, int(lam[p])) for p in list(primerange(2,80))]
print("1c  lam(p)=-1 for ALL p<80:", all(v==-1 for _,v in lp), " (count=%d)"%len(lp))
...
# RE-VERIFY 2: TRACE-SUFF = Lidskii (|trace| <= ||.||_1); diagonal ||.||_1 = x
...
for _ in range(3000):
    ...
    assert abs(abs(np.sum(ev)) - abs(np.trace(A))) < 1e-8          # trace = sum eigenvalues (Lidskii)
    worst = min(worst, float(sv.sum() - abs(np.sum(ev))))           # ||A||_1 - |trace| >= 0
print("2a  min(||A||_1 - |trace|) over 3000 matrices = %.4f  (>=0 confirms |trace|<=||.||_1)" % worst)
N = 200000
c = (lam[1:N+1]*lam[3:N+3]); C5 = float(np.sum(c))
print("2b  DIAGONAL diag(c(n)) n<=%d: trace=%.0f=C5 ; ||.||_1=sum|c|=%d (== N; no savings)"
      % (N, C5, int(np.sum(np.abs(c)))))
print("2c  RANK-ONE C5*(e (x) e): trace=%.0f=C5 ; ||.||_1=|C5|=%.0f  BUT built by RESUMMING sum c(n)"
      % (C5, abs(C5)))

# RE-VERIFY 3: c non-multiplicative on coprimes
import random
...
print("3   c(ab)=c(a)c(b) on coprimes violated %d/%d = %.3f (~half => non-multiplicative)"%(bad,tot,bad/tot))
```

```
1a  (1+p^-s)^-1 == (1-p^-s)/(1-p^-2s)  [=> Euler product = zeta(2s)/zeta(s)]: True
1b  s=2  sum lam(n)/n^s=0.6579736273  zeta(2s)/zeta(s)=0.6579736267  |diff|=5.43e-10
1b  s=3  sum lam(n)/n^s=0.8463351937  zeta(2s)/zeta(s)=0.8463351937  |diff|=0.00e+00
1c  lam(p)=-1 for ALL p<80: True  (count=22)
1d  periods q<=128: []  => NOT a Dirichlet character: True
2a  min(||A||_1 - |trace|) over 3000 matrices = 0.0323  (>=0 confirms |trace|<=||.||_1)
2b  DIAGONAL diag(c(n)) n<=200000: trace=434=C5 ; ||.||_1=sum|c|=200000 (== N; no savings)
2c  RANK-ONE C5*(e (x) e): trace=434=C5 ; ||.||_1=|C5|=434  BUT built by RESUMMING sum c(n)
3   c(ab)=c(a)c(b) on coprimes violated 6431/12904 = 0.498 (~half => non-multiplicative)
```

**Read (independent confirmation).**
- **DDS-CORE facts hold.** The local Euler factor identity is exact; `Σλ(n)n^{−s}` matches `ζ(2s)/ζ(s)` to
  `5.4e−10` at `s=2` and to displayed precision at `s=3`; `λ(p)=−1` for **all** 22 primes `<80`; and λ has
  **no period `q≤128`**. A Dirichlet character mod `q` is `q`-periodic, so **λ is not a Dirichlet character
  of any modulus** — the load-bearing CFT fact. (Note re DISTILL: DISTILL cited the exponent as `ζ(2s)/ζ(s)`
  correctly; the `s=2` value `0.6579…` is `ζ(4)/ζ(2)`, consistent — no discrepancy.)
- **TRACE-SUFF = Lidskii holds.** `min(‖A‖₁−|trace|)=0.0323 ≥ 0` over 3000 non-normal matrices, and
  `trace=Σ eigenvalues` verified to `1e−8` — this is exactly the Lidskii/Weyl-majorant inequality
  `|trace(A)| ≤ Σ|λ_k| ≤ Σσ_k = ‖A‖₁`. The **diagonal** build with `c(n)` on the diagonal has `‖·‖₁ = N`
  **exactly** (no savings); the only small-`‖·‖₁` witness is the rank-one one, which **resums `Σc(n)`**.
- **Non-multiplicativity holds.** `c(ab)=c(a)c(b)` fails on `0.498` (~half) of coprime pairs ⇒ no
  Euler-product / Dirichlet object has `c` as summatory function.

### 0.2 Re-verification of the two non-computational load-bearing facts (authoritative sources)

These are the audits I cannot compute; I confirmed them from authoritative sources rather than trusting the
DISTILL pre-fill.

- **π₁^ét(Spec ℤ) = 1 (Minkowski / Hermite–Minkowski).** Confirmed: MIT 18.785 lecture notes
  (`math.mit.edu/classes/18.785/2017fa/LectureNotes14.pdf`, "The Minkowski bound and finiteness results")
  and arXiv:0803.2096 ("Smallness of fundamental groups for arithmetic schemes"). Minkowski ⇒ ℚ has **no**
  nontrivial everywhere-unramified extension ⇒ every finite étale cover of `Spec ℤ` is trivial ⇒ **there is
  no nontrivial finite étale ℤ/2 local system on `Spec ℤ` at all.** On an open `U = Spec ℤ[1/N]`, finite
  abelian étale covers ↔ finite-order idele-class characters ramified only within `{p|N}` (CFT), i.e. a
  finite ℤ/2 system is an order-2 **Dirichlet** character `(·/D)`; λ is not one (0.1). This is the precise,
  defensible strength of the no-go: it is a statement about **finite** geometric ℤ/2 systems.
- **Connes–Consani arithmetic site's Hasse–Weil zeta = the complete Riemann ζ.** Confirmed: *The Arithmetic
  Site*, Connes–Consani, arXiv:1405.4527 (and C. R. Acad. Sci. Paris 352 (2014) 971–975) — the complete
  Riemann zeta `ζ_ℚ(s)=π^{−s/2}Γ(s/2)ζ(s)` is realized as the Hasse–Weil zeta function of the arithmetic
  site (adele-class-space counting). **This is the escape-hatch finding:** the one F₁/non-finite geometry
  that could carry a "parity sheaf" for λ has ζ **itself** as its zeta function, so realizing λ (Dirichlet
  series `ζ(2s)/ζ(s)`) on it re-imports `ζ(2s)/ζ(s)` — i.e. **Trap A** (the analytic continuation / zeros of
  ζ), **not** a dodge. This is the same obstruction as OPEN node **C7** viewed from the other side, and it
  is exactly the cross-note already recorded on C7 (TRACKING.md, the C7 "A21 cross-check").

### 0.3 Label assignment (the four nodes) + the A21 overall disposition

**[DDS-CORE] → NEEDS-REVIEW (CLAIM GRAPH), as a rigorous wall-FACT, tied to C7.**
- **Statement (at defensible strength):** λ is not a finite-order idele-class (Dirichlet/Hecke) character
  (Dirichlet series `ζ(2s)/ζ(s)`; `λ(p)=−1` ∀p; no period) ⇒ **no nontrivial finite geometric ℤ/2 local
  system over `Spec ℤ` (Minkowski: none exist at all) or over any open `Spec ℤ[1/N]` (CFT: only quadratic
  Dirichlet characters) realizes λ**; λ is **non-automorphic**, and forcing a geometric realization
  re-imports `ζ(2s)/ζ(s)` = Trap A.
- **Why NEEDS-REVIEW not FALSIFIED:** the attack (seek a non-finite / pro-étale / ℓ-adic / stacky /
  arithmetic-site realization of λ that **dodges** Trap A) found **none**, and the arithmetic-site fact
  corroborates the wall from a second direction. There is a real, checkable audit (0.1 computations + 0.2
  authoritative sources). No counterexample exists.
- **Explicitly NOT a foundation toward TPC, NOT a bound, NOT progress.** It is the *sharpened wall-reason*
  for why parity resists geometrization. It **TIES TO / strengthens OPEN node C7**: the parity wall (λ has
  no geometric ℤ/2 realization free of ζ) and the C7 fact (the arithmetic-site zeta **is** ζ, so a λ-sheaf
  there re-imports ζ) are **the same obstruction from two sides**. Cross-link recorded on both nodes.
- **Residual crack (recorded, NOT pursued, = the OPEN part inside C7):** an explicit ℓ-adic / motivic /
  arithmetic-site local system whose Frobenius trace reproduces `λ(n)` at a positive density of primes
  **and** whose construction provably requires no analytic continuation / zeros of ζ. **None known or
  claimed anywhere.** This is OPEN and lives inside C7; "implausible" (the arithmetic-site fact) is not
  "impossible", so the crack is stated, not closed.

**[DDS-ENV] → OPEN (OPEN-CHECKPOINTS).**
- **Statement:** no single structured theory (bialgebra / Hopf-algebroid / Tannakian category / species /
  operad / 2-adic or motivic realization) carries BOTH the non-descending parity character `ε=Σ_p e_p` on
  `⊕_p ℤ` AND the `+2` shift with a compatible trace.
- **Why OPEN not NEEDS-REVIEW:** DISTILL tried to **construct** a counterexample (any traced theory with
  both ε and +2) and found **none**, and has **no impossibility proof**. Per anti-deception, **no
  construction + no impossibility proof = an absence-of-construction / Sarnak-philosophy claim = OPEN.**
  The obstructions in the audit (+2 is not a homomorphism of `⊕_p ℤ` and is a non-polynomial scramble of the
  exponent lattice where ε is native; `c` non-multiplicative) are obstructions to the *obvious* builds, not
  a proof of non-existence over the open-ended class. NOT NEEDS-REVIEW.

**[TRACE-SUFF] → ESTABLISHED-IN-LITERATURE (CLAIM GRAPH), as a SUFFICIENT CONDITION only.**
- **Statement (⟸):** if trace-class `A_x` exist with `trace(A_x)=C5(x)` and `‖A_x‖₁=O(x^{1/2+ε})`, then
  `C5(x)=O(x^{1/2+ε})`.
- **Why ESTABLISHED:** this is **exactly Lidskii's theorem** in inequality form — `trace(A)=Σλ_k(A)`, and
  `|trace(A)| ≤ Σ|λ_k| ≤ Σσ_k = ‖A‖₁` (Weyl majorant). Reference: B. Simon, *Trace Ideals and Their
  Applications*, 2nd ed., AMS Surveys & Monographs 120 (2005), Ch. 3. Re-verified numerically (0.1, item 2a).
- **Explicitly a SUFFICIENT CONDITION, NOT a bound, NOT a foundation.** It does **not** produce the operator
  `A_x`; it is conditional on the unproven existence of `A_x` with small trace norm. It does not, by itself,
  bound C5.

**[TRACE-EQUIV] → OPEN (OPEN-CHECKPOINTS).**
- **Statement (⟹):** a bound `C5(x)=O(x^{1/2+ε})` produces such an `A_x`, evaluated **structurally** (known
  spectrum / forced degeneration), **not** by resumming `Σc(n)`.
- **Why OPEN (precise verdict — not false, not a clean equivalence):** the rank-one `A_x=C5(x)(e⊗e)` makes
  bare existence trivially true, **but is built by resumming `Σc(n)`**, which the structural clause
  **forbids**; DISTILL then computed EXACT singular values of the natural structural builds — the **only**
  build with `trace=C5` is the diagonal (`‖·‖₁=x` exactly, no savings), and Hankel/Toeplitz builds both
  **raise `‖·‖₁`** (~6979/6540 at m=400) **and lose `trace=C5`**. So ⟹ is **UNPROVEN under the structural
  clause** (no known map {bound on C5} → {structural `A_x`}), and there is **no audit for a bound** ⇒ OPEN.
- Net: **`L-TRACE-REFORM` is a ONE-DIRECTIONAL sufficient / equal-hardness reformulation** (⟸ rigorous
  `[TRACE-SUFF]`; ⟹ downgraded `[TRACE-EQUIV]`) — the **`L-QUB-reform` / `L-ERG-RIG`** family. NOT a bound,
  NOT a foundation, NOT progress unless a genuinely structural `A_x` is exhibited.

**A21 / L-DDS / L-TRACE-REFORM — overall disposition.** The from-scratch (L3) attempt **did NOT break
parity; it CLOSED as a source of progress.** Its yield:
- a **partly-rigorous wall-reason** — `[DDS-CORE]` NEEDS-REVIEW (tied to OPEN C7) + `[DDS-ENV]` OPEN;
- an **equal-hardness spectral reformulation** — `[TRACE-SUFF]` ESTABLISHED (sufficient condition) +
  `[TRACE-EQUIV]` OPEN.
`L-DDS` and `L-TRACE-REFORM` are now **resolved out of CANDIDATE LEMMAS** (each split into its two nodes).
**C5 stays OPEN / named, not built upon. Legal foundations UNCHANGED** (C1-membership NEEDS-REVIEW;
C2/C3/C4/C6 ESTABLISHED).

---

## Part 1 — NAMED ACTION: Review

**A21 verdict, stated plainly.** The from-scratch L3 attempt to *construct* a parity-breaking invariant on
C5 **did not break parity.** Six explicit constructions collapsed, unified by the Descent–Definability
Split; DISTILL and this labelling reduce that to four nodes, of which **exactly one is a rigorous fact and
it is a WALL, not a foundation** (`[DDS-CORE]`), one is a **textbook sufficient condition** (`[TRACE-SUFF]`),
and **two are OPEN** (`[DDS-ENV]`, `[TRACE-EQUIV]`). This confirms — from a native / from-scratch direction —
the campaign meta-pattern (A16/A18/A19/A20/A21): **Trap A is dodgeable; Trap B / parity is the universal
wall.** No bound on C5 is certified; no legal foundation changes.

**The notable STRUCTURAL yield (the reason this step is worth recording).** `[DDS-CORE]` gives the project,
for the first time, a **rigorous statement of WHY parity resists geometrization**: the Liouville parity
character λ is **non-automorphic** (its Dirichlet series is `ζ(2s)/ζ(s)`, it has no period, it is not a
Dirichlet/Hecke character), so it is realized by **no** nontrivial finite geometric ℤ/2 local system over
`Spec ℤ` (Minkowski: none exist) or its opens (CFT: only quadratic Dirichlet characters), and **any** attempt
to realize it geometrically — including on the one F₁ geometry that could carry a parity sheaf, the
Connes–Consani arithmetic site — **re-imports `ζ(2s)/ζ(s)` = Trap A**, because that site's Hasse–Weil zeta
**is** the complete Riemann ζ. This **unifies the parity wall with the OPEN C7 obstruction**: the two are the
same obstruction from two sides (the parity-geometrization wall and the "arithmetic-site zeta is ζ" fact).
This cross-link is recorded on **both** nodes (C7 already carried the A21 cross-check; DDS-CORE now records
it symmetrically).

**Three escalation handles (recorded, NOT pursued this step).**
1. **Puncture DDS-CORE:** exhibit an explicit **ℓ-adic / motivic / arithmetic-site λ-realization free of
   ζ-continuation** (Frobenius trace reproducing λ at a positive density of primes, provably needing no
   analytic continuation / zeros of ζ). This would be genuine **progress on C7** (and would break the
   parity wall from the geometric side). None known.
2. **Falsify DDS-ENV:** **construct** a common structured theory (bialgebra / Hopf-algebroid / Tannakian /
   species / operad / motivic) carrying **both** ε and +2 with a compatible trace. A construction falsifies
   the envelope; a Tannakian/Galois **impossibility theorem** would instead upgrade DDS-ENV to a wall-fact.
3. **Settle TRACE-EQUIV / get a bound:** exhibit a genuinely **structural** trace-class `A_x` (known spectrum
   or forced degeneration) with `trace(A_x)=C5(x)` and `‖A_x‖₁=O(x^{1/2+ε})` — no resummation. This would
   settle `[TRACE-EQUIV]` **and give a bound on C5** via `[TRACE-SUFF]`. This line does not do it (diagonal
   costs `‖·‖₁=x`; off-diagonal builds lose `trace=C5`).

**Recommendation: PAUSE.** This is a **checkpoint, not a terminal state** (AGENTS.md: locating/sharpening the
parity obstruction is a checkpoint to break or escalate, never an exit). It is **not progress, not a bound.**
The valuable-but-non-terminal yield is a rigorous geometric-non-realizability core (tied to C7) and a precise
spectral reformulation, both **short of a bound**. The three handles above are the live escalation directions
for a future step; none is opened now.

**Permitted honest sentence (AGENTS.md).** *"I made no progress on C5 that survives audit. A from-scratch
attempt to construct a parity-breaking invariant collapsed; its surviving yield is (i) a rigorous WALL-fact —
the Liouville parity character is non-automorphic, so no finite geometric ℤ/2 local system realizes it and
the one F₁ geometry that could carry a parity sheaf re-imports the Riemann ζ (Trap A), tying the parity wall
to the open C7 obstruction — and (ii) a one-directional / equal-hardness spectral reformulation whose only
rigorous half is the Lidskii sufficient condition. Both are short of a bound. This is a checkpoint, not an
exit."*
